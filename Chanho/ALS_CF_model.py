...
def ALS_CF_cpl(train_file, test_file, result_path, k=2**6, topk=10):
    ...
    with open(os.path.join(result_path, "CF_rec_cpl_dim_{}.pickle".format(k)), 'wb') as f:
        pickle.dump(ingred_recs, f)
        

def ALS_CF_clf(train_file, test_file, result_path, k=2**6, topk=10):
    ...
    with open(os.path.join(result_path, "CF_rec_clf_dim_{}.pickle".format(k)), 'wb') as f:
