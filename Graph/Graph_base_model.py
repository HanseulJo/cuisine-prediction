...
def graph_cpl(train_file, test_file, result_path, result_type, depth, weights, recipe_th, ingred_th, label_th, topk):
    batch_size = 1024
    ...
    for k in range(depth):
        with open(os.path.join(result_path, "Graph_rec_cpl_{}_{}_{}_depth_{}.pickle".format(result_type,w1,w2,k)), 'wb') as f:
            pickle.dump(ingred_recs_depth[k], f)

            
def graph_clf(train_file, test_file, result_path, result_type, depth, weights, recipe_th, ingred_th, label_th, topk):
    batch_size = 1024
    recipe_threshold = recipe_th
    ingred_threshold = ingred_th
    label_threshold = label_th
    ...
    for k in range(depth):
        with open(os.path.join(result_path, "Graph_rec_clf_{}_{}_{}_depth_{}.pickle".format(result_type,w1,w2,k)), 'wb') as f:
